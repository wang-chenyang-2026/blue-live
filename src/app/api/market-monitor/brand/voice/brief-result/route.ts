...
import { ctsGetVoiceBriefResult } from '@/lib/social-mcp';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!task.brief_session_id) {
      return NextResponse.json(
        { success: false, error: '任务缺少 brief session id' },
        { status: 400 },
      );
    }

    const result = await ctsGetVoiceBriefResult(Number(task.brief_session_id));

    let newStatus = task.status;
    if (result.status === 'COMPLETED') {
      // brief 解析完成，但还没提交采集任务，仍为 pending
      newStatus = 'pending';
    ...
    console.error('[brand/voice/brief-result]', msg);
